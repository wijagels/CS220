...
#include "players.h"
	...
	/*--------------------------
	New Idea... create a "pool" for each tier
	The pool consists of a list of registry indexes
	Initialize the pool from the registry to be all valid indexes
	Create an empty pool for the next tier
	Select two players at random from this pool and play them against each other
	These two players are removed from this pool.
	The winner of the match gets added to the next tier pool
	If there is only 1 player left in the pool, he gets a bye, and gets put into the next tier
	If the next tier has only 1 player, he is the tournament winner
	------------------------------------*/
